      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <Field label="Planting date *">
          <input
            ...
            className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-900 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/60 focus:border-emerald-400 transition disabled:opacity-60 disabled:bg-slate-100 disabled:cursor-not-allowed"
            required
            disabled={!!plantRecord}
          />
        </Field>
...
function SensorPanel({ latest, error }) {
  if (error) {
    return (
  ...
  if (!latest) {
    return (
      <div className="rounded-xl border border-slate-700 bg-slate-800/50 p-4 text-sm text-slate-400 italic">
  ...
  return (
    <div className="space-y-3 pt-2">
      <h4 className="text-xs uppercase tracking-wider text-slate-400">Live Environment</h4>
      <div className="grid grid-cols-2 gap-3">
        <SensorCard label="Temp" value={`${latest.temperature?.toFixed(1) ?? "-"} °C`} color="text-amber-300" />
        <SensorCard label="Humidity" value={`${latest.humidity?.toFixed(1) ?? "-"} %`} color="text-sky-300" />
        <SensorCard label="Light" value={`${latest.lux?.toFixed(0) ?? "-"} lx`} color="text-yellow-300" />
        <SensorCard label="Air Quality" value={latest.mq135?.toFixed(0) ?? "-"} color="text-emerald-300" />
      </div>
    </div>
  );
}

function SensorCard({ label, value, color }) {
  return (
