      ...
      >
        ...
        <div className="relative flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="space-y-3 max-w-3xl">
            <p className="text-xs uppercase tracking-[0.3em] text-slate-400">Growth intelligence</p>
            <h2 className="text-3xl font-semibold leading-tight">Feed planting dates and heights. Get instant ML guidance.</h2>
            <p className="text-slate-300 text-sm md:text-base">
              The FastAPI endpoint <code className="px-2 py-1 rounded bg-slate-800/80 border border-slate-700">/analyze-growth</code> returns
              a growth classification, confidence scores, and the expected height range for the plant's age.
            </p>
            <div className="flex flex-wrap gap-3 text-xs text-slate-300">
              <Badge>Model: Random Forest</Badge>
              <Badge>Features: age_days + plant_height_mm</Badge>
              <Badge>Output: label + probabilities</Badge>
            </div>
          </div>
          <div className="rounded-2xl border border-emerald-500/40 bg-gradient-to-br from-emerald-500/20 via-emerald-400/10 to-blue-500/20 px-4 py-5 shadow-lg shadow-emerald-500/25 text-sm text-emerald-50">
            <p className="uppercase text-[10px] tracking-[0.3em] text-emerald-100/80 mb-2">API payload</p>
            <pre className="text-xs leading-relaxed whitespace-pre-wrap">
{`{
  "planting_date": "YYYY-MM-DD",
  "current_height_mm": 24.5,
      ...
      <div className="grid lg:grid-cols-5 gap-6 items-start">
        ...
        >
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <Field label="Current date (optional)">
              <input
                type="date"
                value={currentDate}
          ...
          <div className="flex flex-wrap gap-3 text-xs text-slate-400">
            <p>Tip: current_date defaults to today if left empty.</p>
                ...
                >
                  ...
                  <div className="grid md:grid-cols-3 gap-4">
                    <StatCard
                      ...
                      hint="Difference between planting_date and current_date"
                    />
                    <StatCard
                      title="Expected range (mm)"
                      value={result.expected_height_range ? `${result.expected_height_range[0]} - ${result.expected_height_range[1]}` : '-'}
                      hint="Benchmark from heuristic ranges"
                    />
                    <StatCard
                      title="Current height (mm)"
                      value={Number(currentHeight) || result?.plant_height_mm || '-'}
                      hint="Value you provided"
                    />
                  </div>
...
function Badge({ children }) {
  return (
    <span className="inline-flex items-center gap-1 rounded-full border border-slate-700/80 bg-slate-800/60 px-3 py-1 text-[11px] text-slate-300">
      {children}
    </span>
  );
}
