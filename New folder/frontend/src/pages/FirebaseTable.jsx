...
export default function FirebaseTable() {
  ...
  return (
    <div className="space-y-6">
      <div className="glass rounded-3xl p-6 border border-fuchsia-400/30">
        <p className="text-xs uppercase tracking-[0.25em] text-slate-400">Firebase RTDB</p>
        <h2 className="text-2xl font-semibold">Live plant table</h2>
        <p className="text-slate-300 mt-2">Proxy CRUD via the backend to the configured Realtime Database.</p>
      </div>

      <div className="glass rounded-3xl p-6 border border-white/10 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="text-sm text-slate-300">Rows: {filtered.length}</div>
          <div className="flex gap-2">
            <input
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              placeholder="Search by Jar/Plant ID"
              className="rounded-xl bg-slate-900/70 border border-slate-800 px-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-emerald-500/60"
            />
            <button
              onClick={fetchData}
              className="rounded-xl border border-white/10 px-3 py-2 text-sm hover:border-emerald-400/60"
              disabled={loading}
            >
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 p-4 space-y-3">
          <p className="text-sm font-semibold text-slate-200">Create/Update record</p>
          <form onSubmit={onSave} className="grid md:grid-cols-4 sm:grid-cols-2 gap-3">
            <input
              ...
              placeholder="Jar ID *"
              className="rounded-xl bg-slate-900/70 border border-slate-800 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/60"
            />
            <input
              type="date"
